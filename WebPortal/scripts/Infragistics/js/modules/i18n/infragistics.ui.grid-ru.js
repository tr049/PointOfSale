...
if (!$.ig.Grid) {
	...
	$.extend($.ig.GridGroupBy, {
		locale: {
			...
			summaryIconTitle: "Сводка для {0}: {1}"
		}
	});
	...
	$.extend($.ig.GridRowSelectors, {
		...
		locale: {
			...
			columnVirtualizationEnabled: "Если включена виртуализация столбцов, опция igGridRowSelectors не поддерживается. Используйте опцию rowVirtualization или установите для параметра virtualizationMode значение 'continuous'.",
	...
	$.extend($.ig.GridUpdating, {
		locale: {
			...
			virtualizationNotSupportedWithoutAutoCommit: "Пока деактивирована опция autoCommit, включение обновления и виртуализации не поддерживается. Активируйте опцию сетки autoCommit."
		}
	});
    ...
    $.extend($.ig.ColumnMoving, {
        locale: {
            ...
            columnVirtualizationEnabled: "Для перемещения столбца требуется другая установка виртуализации. Используйте опцию rowVirtualization или установите для параметра virtualizationMode значение 'continuous'."
        }
    });
    ...
    $.extend($.ig.igGridMultiColumnHeaders, {
    	locale: {
    	    ...
    	    atLeastOneColumnShouldBeShownWhenCollapseOrExpand: 'При растягивании или сжатии заголовка для нескольких столбцов у вас должен отображаться хотя бы один столбец.',
    	    collapsedColumnIconTooltip: "Растянуть",
    	    expandedColumnIconTooltip: "Сжать"
    	}
    });
